...
"""
SQLite storage (`sqlite_path` / `SQLITE_PATH`).

**Local dev** default. On **Lambda** (without Aurora), the file lives under `/tmp` and is not
a shared server database. Prefer **Aurora + `postgres` backend** for production deploys.
"""
...
def dashboard_aggregates(*, user_id: str) -> dict[str, int | float]:
    """Pre-computed dashboard numbers for a user."""
    conn = get_conn()
    try:
        n_apps = conn.execute(
